...
この設計は runtime の境界として扱う。現在の micro-pilot では、touch / ripple を直接音量や低域へ接続せず、まず safe ambient concept に変換する。
...
- high gesture energy は `water_shimmer` / `air_lift` / rare `melody_fragment` へ安全に丸め、master loudness や low-end pressure を直接上げない

## 10. Next suggested PRs

1. 1回タップ/スワイプあたりの期待値（ripple decay time / max rise / max fade）を listening review で確認する
2. `water_day` / `garden_morning` / `family_room` の安全な体感差を人間レビューで固定する
3. `ambient-listening-scorecard.md` と runtime snapshot の対応を整理する
